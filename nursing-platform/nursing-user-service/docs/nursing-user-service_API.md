...
接口说明：受理注册、登录或重置密码验证码发送任务。请求必须在 `Idempotency-Key` 请求头中携带客户端生成的 UUID；同一用户操作的网络重试必须复用该值。接口受理成功后前端可直接开始倒计时，无需轮询发送状态。
...
| 字段 | 必填 | 规则 | 说明 |
|---|---|---|---|
| `Idempotency-Key` | 是 | UUID | 同一次发送请求及其重试使用同一个值 |
...
| HTTP 状态 | code | 说明 |
|---:|---:|---|
| 200 | 0 | 发送任务已受理；`status` 可能为 `PENDING`、`PROCESSING`、`SENT_ACCEPTED`、`FAILED` 或 `UNKNOWN` |
| 400 | 1000 | 参数校验失败，包含缺失或非 UUID 格式的 `Idempotency-Key` |
| 409 | 2019 | 同一个 `Idempotency-Key` 被用于不同手机号或短信类型 |
...
| Item | Contract |
|---|---|
| Method and path | `POST /internal/v1/sms/provider-receipts` |
| Caller | SMS provider callback service or a trusted internal reconciliation service only |
| Authentication | An approved provider-signature verifier or service-to-service credential is mandatory. Requests without a configured verifier are rejected and the route is not exposed. |
| Idempotency | `providerRequestId + deliveryStatus + receiptTime` identifies a logical receipt. Repeated equivalent receipts must not create duplicate transitions. |
...
```json
{
  ...
  "receiptTime": "2026-07-13T09:30:00+08:00",
  "receiptSummary": "provider-defined bounded receipt summary"
}
```

| Field | Required | Rules |
|---|---:|---|
| `providerRequestId` | yes | 1-128 characters; must identify an existing provider request |
| `deliveryStatus` | yes | `DELIVERED` or `DELIVERY_FAILED` |
| `receiptTime` | yes | ISO-8601 timestamp |
| `receiptSummary` | no | At most 1024 characters; must not include the SMS code or provider credentials |
