...
本文描述当前工作树已实现的行为，不补充真实短信、真实支付、管理端或跨库分布式事务等尚未实现能力。事实来源为各服务的 Controller、Service、Mapper XML、初始化 DDL、Flyway 迁移和 `application*.yml`；接口路径均以网关入口为准，内部接口除外。

- 服务：网关 `8080`，用户 `8081`，目录 `8082`，订单 `8083`，反馈 `8084`；服务通过 Nacos（`NURSING` group）发现与读取按 `${service}-${profile}.yaml` 命名的配置。
- 持久化：服务分别拥有 `user_db`、`catalog_db`、`order_db`、`feedback_db`，没有业务外键或跨库事务。订单、反馈启用 Flyway；用户、目录仅使用各自 `deploy/mysql/init` 初始化脚本。
- 公共模块：`nursing-common` 提供统一 `Result`、错误码、雪花 ID、领域 DTO 和 Feign 声明。订单通过 `CatalogServiceFeignClient` 查询目录；反馈通过 `OrderFeignClient` 查询订单内部接口。
...
1. 订单、反馈的 `deploy/mysql/init` 仍是基础脚本；运行配置启用 Flyway、`baseline-on-migrate=true`、`baseline-version=1`。最终形态需要基础结构加 V2：订单增加 `payment_intent`、`refund_record` 和支付通知/交易号唯一键；反馈增加评价项目名/规格名快照列和缺快照索引。对已由旧脚本创建而未受 Flyway 管理的库，是否实际执行 V2 取决于 Flyway 历史表及部署迁移流程，不能仅凭基础 DDL 认定完整。
2. 订单 DDL 与当前 Mapper/实体存在历史演进风险：基础 `order_header` 的部分唯一索引、Outbox/支付字段以当前 V1 Mapper/迁移为准；部署前应通过 `docker-compose/db-bootstrap/schema-preflight.sh` 和 Flyway 校验，而不是混用已删除的旧初始化脚本。
