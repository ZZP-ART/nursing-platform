...
| `nursing-common` | 共享库 | 响应模型、异常、Snowflake ID、DTO、订单支付事件契约 | 被业务服务依赖 |
...
订单和反馈服务的初始 SQL 包含支付意图、退款可靠性和评价快照所需结构。首次容器启动时，MySQL 初始化脚本负责创建所有业务库和 Nacos 库；`db-bootstrap` 创建最小权限业务账号后服务再启动。
...
- 初始化任务：`db-bootstrap`、`uploads-init`。
...
服务的 `application.yml` 仅包含服务标识、Profile 和 Nacos 连接信息；业务基础配置从 Nacos 的 `NURSING` 分组加载 `${spring.application.name}.yaml`，环境覆盖从 `${spring.application.name}-${profile}.yaml` 加载。连接地址和密钥通过环境变量注入。Compose 由 `nacos-config-bootstrap` 在服务启动前调用 Nacos API 导入基础与当前 Profile 的非敏感模板；Nacos 配置缺失会阻止服务启动，不再使用 `optional:` 本地兜底。
...
| 支付/退款 | Nacos 环境配置中的 `nursing.payment.mock`、`NURSING_ALIPAY_*`、`NURSING_REFUND_*` |
